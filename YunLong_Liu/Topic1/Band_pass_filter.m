...
samplFreq = 1024*2;%5*maxFreq;%样本频率
samplIntrvl = 1/samplFreq;%样本时间间隔
nSamples= 2048*2;%samplFreq;%样本数量
...
% Design Band pass filter 带通滤波器设计
filtOrdr = 50;
% 横轴频率空间范围 samplFreq/2
% 归一化 1/(samplFreq/2)
b = fir1(filtOrdr,[((f1+f2)/2)/(samplFreq/2),((f2+f3)/2)/(samplFreq/2)],'high');
F0=figure;
freqz(b,1)
title('Bandpass Filtered Design')
saveas(F0,'BandpassDesign.jpg');
% Apply filter
